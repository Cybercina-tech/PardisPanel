# مستندات ارسال قیمت‌ها به API خارجی

این مستندات نحوه ارسال چهار قیمت (GBP و USDT - خرید و فروش) به API وب‌سایت صرافی پردیس را توضیح می‌دهد.

## 📋 فهرست مطالب

- [نمای کلی](#نمای-کلی)
- [قیمت‌های ارسالی](#قیمت‌های-ارسالی)
- [ساختار API](#ساختار-api)
- [نحوه کار](#نحوه-کار)
- [تنظیمات](#تنظیمات)
- [مثال‌های استفاده](#مثال‌های-استفاده)
- [عیب‌یابی](#عیب‌یابی)

## نمای کلی

سیستم Pardis Panel به صورت خودکار قیمت‌های نهایی‌شده (finalized) را به API خارجی وب‌سایت `https://sarafipardis.co.uk` ارسال می‌کند. این ارسال در دو حالت انجام می‌شود:

1. **Finalize کردن یک Category**: هنگام finalize کردن قیمت‌های یک دسته‌بندی (مثلاً تتر یا پوند)
2. **Finalize کردن Special Price**: هنگام finalize کردن یک قیمت ویژه

### فایل‌های مرتبط

- `finalize/services.py`: شامل کلاس `ExternalAPIService` که مسئول ارسال قیمت‌ها است
- `finalize/views.py`: شامل view های finalize که از سرویس استفاده می‌کنند

## قیمت‌های ارسالی

سیستم فقط چهار قیمت زیر را به API ارسال می‌کند:

| کلید | توضیحات | مثال | منبع |
|------|---------|------|------|
| `GBP_BUY` | قیمت خرید پوند به تومان | 163,000 | **فقط پوند حسابی** (نه نقدی) |
| `GBP_SELL` | قیمت فروش پوند به تومان | 172,000 | **فقط پوند حسابی** (نه نقدی) |
| `USDT_BUY` | قیمت خرید تتر به تومان | 126,000 | تتر به تومان |
| `USDT_SELL` | قیمت فروش تتر به تومان | 150,000 | تتر به تومان |

### نکات مهم

- **برای پوند (GBP)**: فقط قیمت‌های "حسابی" (account) ارسال می‌شوند. قیمت‌های "نقدی" (cash) نادیده گرفته می‌شوند
  - سیستم بررسی می‌کند که آیا کلمه "حسابی" یا "account" در نام `PriceType` وجود دارد
  - اگر وجود نداشته باشد، آن قیمت skip می‌شود
  
- **برای تتر (USDT)**: فقط قیمت‌های به تومان (IRR) ارسال می‌شوند
  - جفت ارز باید `USDT/IRR` یا `IRR/USDT` باشد
  
- فقط قیمت‌های مربوط به جفت ارزهای `GBP/IRR` (یا `GBP/IRT`) و `USDT/IRR` (یا `USDT/IRT`) ارسال می‌شوند
- قیمت‌های سایر ارزها (مثل USD, EUR و غیره) نادیده گرفته می‌شوند
- نوع معامله (`trade_type`) باید `buy` یا `sell` باشد

## ساختار API

### Endpoint

```
POST https://sarafipardis.co.uk/wp-json/pardis/v1/rates
```

### درخواست (Request)

برای هر قیمت، یک درخواست POST جداگانه ارسال می‌شود:

```json
{
  "currency": "USDT_SELL",
  "rate": 150000.0,
  "api_key": "PX9k7mN2qR8vL4jH6wE3tY1uI5oP0aS9dF7gK2mN8xZ4cV6bQ1wE3rT5yU8iO0pL"
}
```

#### پارامترها

- `currency` (string, required): یکی از چهار کلید (`GBP_BUY`, `GBP_SELL`, `USDT_BUY`, `USDT_SELL`)
- `rate` (float, required): مقدار قیمت به صورت عدد اعشاری
- `api_key` (string, required): کلید API برای احراز هویت

#### Headers

```
Content-Type: application/json
```

### پاسخ (Response)

API ممکن است یک پاسخ JSON برگرداند یا هیچ چیزی برنگرداند. در هر دو حالت، اگر status code 200 باشد، ارسال موفق در نظر گرفته می‌شود.

### خواندن قیمت‌های موجود (GET)

برای خواندن قیمت‌های فعلی از API:

```
GET https://sarafipardis.co.uk/wp-json/pardis/v1/rates
```

پاسخ نمونه:

```json
{
  "GBP_BUY": 163000,
  "GBP_SELL": 172000,
  "USDT_BUY": 126000,
  "USDT_SELL": 150000
}
```

## نحوه کار

### 1. استخراج قیمت‌ها از Price Items

هنگامی که یک category finalize می‌شود، تابع `send_finalized_prices()` فراخوانی می‌شود:

```python
price_items = [
    (price_type, price_history),
    # ... سایر قیمت‌ها
]

results = ExternalAPIService.send_finalized_prices(price_items)
```

تابع `_build_rates_from_items()` قیمت‌ها را استخراج می‌کند:

1. برای هر `(price_type, price_history)`:
   - کد ارز مبدأ و مقصد را می‌خواند
   - نوع معامله (`buy` یا `sell`) را می‌خواند
   - مقدار قیمت را از `price_history.price` می‌خواند
   - اگر جفت ارز `GBP/IRR` یا `USDT/IRR` باشد، کلید مناسب را می‌سازد

### 2. ساخت کلیدها

```python
# برای GBP - فقط حسابی (account)
if pair == {"GBP", "IRR"} or pair == {"GBP", "IRT"}:
    # بررسی می‌کند که آیا "حسابی" یا "account" در نام price_type وجود دارد
    is_account = "حسابی" in price_type.name or "account" in price_type.name.lower()
    
    if is_account:
        key = "GBP_BUY" if trade_type == "buy" else "GBP_SELL"
    else:
        # پوند نقدی را skip می‌کند
        continue

# برای USDT - فقط به تومان
elif pair == {"USDT", "IRR"} or pair == {"USDT", "IRT"}:
    key = "USDT_BUY" if trade_type == "buy" else "USDT_SELL"
```

### 3. ارسال قیمت‌ها

تابع `send_rates_payload()` چهار قیمت را ارسال می‌کند:

1. **خواندن قیمت‌های موجود**: ابتدا قیمت‌های فعلی از API خوانده می‌شوند
2. **ترکیب قیمت‌ها**: 
   - اگر قیمت جدید برای یک کلید وجود دارد، از آن استفاده می‌شود
   - اگر قیمت جدید وجود ندارد، از قیمت موجود در API استفاده می‌شود
   - اگر هیچ کدام وجود ندارد، مقدار 0 استفاده می‌شود
3. **ارسال جداگانه**: برای هر یک از چهار کلید، یک درخواست POST جداگانه ارسال می‌شود

### 4. مثال جریان کار

```
1. کاربر category "تتر" را finalize می‌کند
   ↓
2. price_items شامل:
   - (USDT_BUY_price_type, price_history_1) → قیمت: 126,000
   - (USDT_SELL_price_type, price_history_2) → قیمت: 150,000
   ↓
3. _build_rates_from_items() استخراج می‌کند:
   {
     "USDT_BUY": 126000.0,
     "USDT_SELL": 150000.0
   }
   ↓
4. send_rates_payload() خواندن قیمت‌های موجود:
   {
     "GBP_BUY": 163000,
     "GBP_SELL": 172000,
     "USDT_BUY": 125000,  # قدیمی
     "USDT_SELL": 149000   # قدیمی
   }
   ↓
5. ترکیب:
   {
     "GBP_BUY": 163000,      # از existing
     "GBP_SELL": 172000,     # از existing
     "USDT_BUY": 126000,     # از new (overwrite)
     "USDT_SELL": 150000     # از new (overwrite)
   }
   ↓
6. ارسال 4 درخواست POST:
   POST /rates {"currency": "GBP_BUY", "rate": 163000, ...}
   POST /rates {"currency": "GBP_SELL", "rate": 172000, ...}
   POST /rates {"currency": "USDT_BUY", "rate": 126000, ...}
   POST /rates {"currency": "USDT_SELL", "rate": 150000, ...}
```

## تنظیمات

### متغیرهای محیطی (Environment Variables)

می‌توانید از متغیرهای محیطی برای تنظیم URL و API Key استفاده کنید:

```bash
# URL API
export EXTERNAL_API_URL="https://sarafipardis.co.uk/wp-json/pardis/v1/rates"

# API Key
export EXTERNAL_API_KEY="your-api-key-here"
```

### تنظیمات پیش‌فرض

اگر متغیرهای محیطی تنظیم نشده باشند، از مقادیر پیش‌فرض استفاده می‌شود:

```python
EXTERNAL_API_URL = "https://sarafipardis.co.uk/wp-json/pardis/v1/rates"
EXTERNAL_API_KEY = "PX9k7mN2qR8vL4jH6wE3tY1uI5oP0aS9dF7gK2mN8xZ4cV6bQ1wE3rT5yU8iO0pL"
```

## مثال‌های استفاده

### استفاده مستقیم از سرویس

```python
from finalize.services import ExternalAPIService

# ارسال قیمت خرید تتر
result = ExternalAPIService.send_usdt_buy(126000.0)

# ارسال قیمت فروش تتر
result = ExternalAPIService.send_usdt_sell(150000.0)

# ارسال قیمت خرید پوند
result = ExternalAPIService.send_gbp_buy(163000.0)

# ارسال قیمت فروش پوند
result = ExternalAPIService.send_gbp_sell(172000.0)
```

### ارسال همه قیمت‌ها

```python
from finalize.services import ExternalAPIService

# ساخت dict قیمت‌ها
rates = {
    "GBP_BUY": 163000.0,
    "GBP_SELL": 172000.0,
    "USDT_BUY": 126000.0,
    "USDT_SELL": 150000.0
}

# ارسال
results = ExternalAPIService.send_rates_payload(rates)
```

### استفاده در View

```python
from finalize.services import ExternalAPIService

# در finalize_category view
price_items = [
    (price_type_1, price_history_1),
    (price_type_2, price_history_2),
    # ...
]

# ارسال خودکار
results = ExternalAPIService.send_finalized_prices(price_items)

# بررسی نتایج
if results.get("sent"):
    print(f"Successfully sent {len(results['sent'])} rates")
if results.get("failed"):
    print(f"Failed to send {len(results['failed'])} rates")
```

## عیب‌یابی

### لاگ‌ها

سیستم لاگ‌های مفصلی برای ردیابی ارسال قیمت‌ها دارد:

```python
# لاگ‌های مهم:
logger.info("send_finalized_prices called with X items")
logger.info("Processing USDT item: source=USDT, target=IRR, trade_type=sell, price=150000")
logger.info("USDT_SELL price extracted: ...")
logger.info("Built rates dict: {...}, skipped: [...]")
logger.info("Existing rates from API: {...}")
logger.info("New rates to send: {...}")
logger.info("Using new rate for USDT_SELL: 150000.0")
logger.info("Successfully sent USDT_SELL rate: 150000")
```

### مشکلات رایج

#### 1. قیمت فروش تتر عدد 1 را نشان می‌دهد

**علت**: قیمت فروش تتر در `price_items` نیست یا به درستی استخراج نمی‌شود.

**راه‌حل**:
- بررسی کنید که `PriceType` برای فروش تتر با `trade_type='sell'` و جفت ارز `USDT/IRR` وجود دارد
- لاگ‌ها را بررسی کنید تا ببینید آیا قیمت فروش تتر در `price_items` است
- بررسی کنید که `trade_type` به درستی `"sell"` است (نه `"Sell"` یا `"SELL"`)

#### 2. قیمت پوند از نقدی استفاده می‌شود به جای حسابی

**علت**: سیستم قبلاً از هر قیمت پوندی استفاده می‌کرد.

**راه‌حل**:
- اطمینان حاصل کنید که `PriceType` برای پوند شامل کلمه "حسابی" یا "account" در نام خود است
- قیمت‌های پوند نقدی به صورت خودکار skip می‌شوند
- لاگ‌ها را بررسی کنید تا ببینید کدام قیمت‌ها skip شده‌اند

#### 2. قیمت ارسال نمی‌شود

**علت**: خطا در ارتباط با API یا مشکل در API Key.

**راه‌حل**:
- بررسی لاگ‌ها برای خطاهای HTTP
- بررسی صحت API Key
- بررسی دسترسی به اینترنت و URL API

#### 3. قیمت‌های قدیمی overwrite نمی‌شوند

**علت**: قیمت جدید در `rates` نیست و از `existing` استفاده می‌شود.

**راه‌حل**:
- بررسی کنید که قیمت جدید در `price_items` است
- بررسی کنید که جفت ارز و `trade_type` به درستی تشخیص داده می‌شوند

### تست

برای تست ارسال قیمت‌ها، می‌توانید از تست‌های نوشته شده استفاده کنید:

```bash
# اجرای تست‌ها
python manage.py test finalize.tests.ExternalAPIServiceTest

# تست خاص ارسال قیمت فروش تتر
python manage.py test finalize.tests.ExternalAPIServiceTest.test_send_usdt_sell_price_150000
```

## نکات مهم

1. **ارسال جداگانه**: برای هر یک از چهار قیمت، یک درخواست POST جداگانه ارسال می‌شود (نه یک درخواست با همه قیمت‌ها)

2. **Fallback به قیمت‌های موجود**: اگر قیمت جدید برای یک کلید وجود ندارد، از قیمت موجود در API استفاده می‌شود

3. **Timeout**: timeout برای درخواست‌ها 30 ثانیه است

4. **Error Handling**: اگر ارسال یک قیمت با خطا مواجه شود، سایر قیمت‌ها همچنان ارسال می‌شوند

5. **Logging**: همه عملیات لاگ می‌شوند، بنابراین می‌توانید روند ارسال را ردیابی کنید

## ساختار کد

```
finalize/
├── services.py          # کلاس ExternalAPIService
│   ├── send_request()           # ارسال یک قیمت
│   ├── get_existing_rates()     # خواندن قیمت‌های موجود
│   ├── send_rates_payload()     # ارسال چهار قیمت
│   ├── _build_rates_from_items() # استخراج قیمت‌ها
│   └── send_finalized_prices()  # ارسال قیمت‌های finalized
└── views.py             # استفاده از سرویس در views
```

## پشتیبانی

برای سوالات یا مشکلات، لاگ‌های سیستم را بررسی کنید یا با تیم توسعه تماس بگیرید.

